...
public:
  ...
  // The returned CompilerType is used to get the type name for formatter
  // matching. Currently returns the type wrapped in a CompilerType without
  // modification.
  CompilerType GetTypeForFormatters(void *type) override;

  Reference<OxCamlType> *RegisterType(lldb::user_id_t die_id,
  ...
  bool CanPassInRegisters(const CompilerType &type) override {
    // OCaml passes values in registers
    return true;
  }
  ...
  lldb::TypeClass GetTypeClass(lldb::opaque_compiler_type_t type) override {
    // OCaml values are built-in types
  ...
  lldb::Encoding GetEncoding(lldb::opaque_compiler_type_t type,
    ...
    // CR sspies: This is not really correct, but doesn't seem to matter.
    count = 1;
  ...
  lldb::Format GetFormat(lldb::opaque_compiler_type_t type) override {
    // CR sspies: This seems to be unused. It currently doesn't
    // impact our formatters.
  ...
  void DumpTypeDescription(lldb::opaque_compiler_type_t type, Stream &s,
                           lldb::DescriptionLevel level) override;
...
private:
  ...
  // Type registry with recursive type support: maps DIE ID to
  // Reference<OxCamlType>
  //
  // This registry enables handling of recursive types in DWARF parsing by
  // providing stable references that can be updated atomically. When parsing
  // recursive types like "type t = A | B of t", a placeholder is first created
  // and all references point to it, then the placeholder is replaced with the
  // actual type once parsing completes.
  //
  ...
  std::unordered_map<lldb::user_id_t, std::unique_ptr<Reference<OxCamlType>>>
      m_type_registry;
};
