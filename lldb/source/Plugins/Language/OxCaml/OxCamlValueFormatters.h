...
/// Format an OCaml double array (float array with Double_array_tag).
/// \param stream Output stream to write to
/// \param array_ptr Address of the array data (first element, not header)
/// \param wosize Number of double elements in the array
/// \param process_sp Process for memory access
/// \returns true if formatting succeeded, false otherwise
bool FormatOxCamlDoubleArray(Stream &stream, lldb::addr_t array_ptr,
                             uint64_t wosize, lldb::ProcessSP process_sp);
