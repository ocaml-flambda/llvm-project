...
constexpr uint64_t HEADER_WOSIZE_MASK =
    (((1ULL << HEADER_WOSIZE_BITS) - 1ULL) << HEADER_WOSIZE_SHIFT);

/// Bit mask for checking LSB (least significant bit) to discriminate immediate
/// vs pointer
...
// CR sspies: Once we have short floats, we should add the suffix below.
/// Float type suffixes (unboxed types)
constexpr const char *FLOAT16_SUFFIX = "";  // float16# -> #3.14
constexpr const char *FLOAT32_SUFFIX = "s"; // float32# -> #3.14s
constexpr const char *FLOAT64_SUFFIX = "";  // float# -> #3.14
...
inline std::string GetUnsignedIntegerSuffix(uint64_t byte_size) {
  ...
  case 1:
    return UINT8_SUFFIX;
  case 2:
    return UINT16_SUFFIX;
  case 4:
    return UINT32_SUFFIX;
  case 8:
    return UINT64_SUFFIX;
  default:
    return ""; // fallback
  }
}
...
/// \return true if value is an immediate (tagged integer, etc.), false if
/// pointer
inline bool IsImmediate(uint64_t value) {
  return (value & constants::VALUE_LSB_MASK) == 1;
}

/// Check if an OCaml value is a pointer (LSB = 0)
/// \param value The OCaml value to check
/// \return true if value is a heap pointer, false if immediate
inline bool IsPointer(uint64_t value) {
  return (value & constants::VALUE_LSB_MASK) == 0;
}
...
inline uint64_t ExtractWosize(uint64_t header) {
  return (header & constants::HEADER_WOSIZE_MASK) >>
         constants::HEADER_WOSIZE_SHIFT;
}
...
inline uint64_t ExtractScannableWosize(uint8_t reserved,
                                       uint64_t total_wosize) {
  if (reserved > 0) {
...
inline uint64_t ExtractNonScannableWosize(uint8_t reserved,
                                          uint64_t total_wosize) {
...
inline void ParseHeader(uint64_t header, uint8_t &tag, uint64_t &wosize,
                        uint8_t &reserved) {
...
inline void ParseHeaderWithMixedBlocks(uint64_t header, uint8_t &tag,
                                       uint64_t &wosize,
...
inline lldb::addr_t GetLastWordAddress(lldb::addr_t block_ptr,
                                       uint64_t wosize) {
